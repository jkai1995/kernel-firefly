...
#define ENABLE_LOW_POWER               (0x3f << 8) //52053e1e: 0xf<<8 error
...
static void dw_mipi_dsi_set_mode(struct dw_mipi_dsi *dsi,																												   enum dw_mipi_dsi_mode mode);
					 ...
					 const struct mipi_dsi_msg *msg)
{
	...
	case MIPI_DSI_GENERIC_LONG_WRITE:
		if (firefly_long_pkt_wrt_switch)
			...
			ret = rockchip_dsi_send_packet(dsi, msg);
		break;
	default:
